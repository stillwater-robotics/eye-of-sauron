...
class StillwaterControl:
    def __init__(self, root):
        ...
        # Manual Drive State
        self.manual_drive = tk.BooleanVar(value=False)
        self.active_keys = {'w': False, 'a': False, 's': False, 'd': False}
        self.md_repeat_job = None # Tracks the 1-second interval loop
    ...
    def build_ui(self):
        ...
        self.lbl_md = tk.Label(self.sidebar, text="Manual Drive", font=("Inter", 11, "bold"))
        self.lbl_md.pack(anchor=tk.W, pady=(0, 10))

        self.md_toggle = tk.Checkbutton(self.sidebar, text="Enable WASD Drive", variable=self.manual_drive, 
                                        font=("Inter", 10), command=self.on_md_toggle)
        self.md_toggle.pack(anchor=tk.W, pady=(0, 10))

        # Keyboard Visualizer (Standard inverted T layout)
        self.md_viz_frame = tk.Frame(self.sidebar)
        self.md_viz_frame.pack(fill=tk.X, pady=5)
        
        self.md_viz_frame.columnconfigure(0, weight=1)
        self.md_viz_frame.columnconfigure(1, weight=1)
        self.md_viz_frame.columnconfigure(2, weight=1)

        self.w_lbl = tk.Label(self.md_viz_frame, text="W", font=("JetBrains Mono", 14, "bold"), width=3, relief="ridge", pady=5)
        self.w_lbl.grid(row=0, column=1, pady=2)
        self.a_lbl = tk.Label(self.md_viz_frame, text="A", font=("JetBrains Mono", 14, "bold"), width=3, relief="ridge", pady=5)
        self.a_lbl.grid(row=1, column=0, padx=2)
        self.s_lbl = tk.Label(self.md_viz_frame, text="S", font=("JetBrains Mono", 14, "bold"), width=3, relief="ridge", pady=5)
        self.s_lbl.grid(row=1, column=1, padx=2)
        self.d_lbl = tk.Label(self.md_viz_frame, text="D", font=("JetBrains Mono", 14, "bold"), width=3, relief="ridge", pady=5)
        self.d_lbl.grid(row=1, column=2, padx=2)
        ...
        self.theme_btn.pack(fill=tk.X, side=tk.BOTTOM)
    ...
    def on_md_toggle(self):
        if self.manual_drive.get():
            self.root.focus_set() # Take focus so typing doesn't happen during drive
        else:
            # Kill the loop and stop the robot if toggled off mid-drive
            if self.md_repeat_job:
                self.root.after_cancel(self.md_repeat_job)
                self.md_repeat_job = None
            self.active_keys = {k: False for k in self.active_keys}
            self.update_md_viz()
            self.send_command(raw_cmd="in 0 0 0")

    def key_press(self, event):
        if not self.manual_drive.get(): return
        k = event.keysym.lower()
        if k in self.active_keys:
            if not self.active_keys[k]:
                self.active_keys[k] = True
                self.update_md_viz()
                # If this is the first key pressed, start the 1-second transmission loop
                if self.md_repeat_job is None:
                    self.transmit_md_command()
    ...
    def key_release(self, event):
        ...
        if k in self.active_keys:
            if self.active_keys[k]:
                ...
                # If no keys are being pressed anymore, stop the loop and send a stop command
    ...
    def transmit_md_command(self):
        left, right = 0, 0
        
        # Determine velocities
        if self.active_keys['w']:
            left, right = 6, 6
        elif self.active_keys['s']:
            left, right = -6, -6
        elif self.active_keys['a']:
            left, right = -6, 6
        elif self.active_keys['d']:
            left, right = 6, -6
            
        if left != 0 or right != 0:
            self.send_command(raw_cmd=f"in {left} {right} 0")
            # Schedule the next transmission in 1000ms (1 second)
            self.md_repeat_job = self.root.after(1000, self.transmit_md_command)
    ...
    def apply_theme(self):
        ...
        # Labels
        for lbl in [self.title_label, self.lbl_robot, self.lbl_tport, self.lbl_lport, self.feed_label, self.hint_label, self.lbl_md]:
            lbl.configure(bg=t["sidebar"] if lbl.master == self.sidebar else t["main"], fg=t["text"])
            
        # Checkbutton Theme
        self.md_toggle.configure(bg=t["sidebar"], fg=t["text"], selectcolor=t["entry_bg"], 
                                 activebackground=t["sidebar"], activeforeground=t["text"])
        self.md_viz_frame.configure(bg=t["sidebar"])
    ...
    def update_md_viz(self):
        t = THEMES[self.current_theme]
        active_bg = t["accent"]
        active_fg = "white"
        inactive_bg = t["entry_bg"]
        inactive_fg = t["text"]
        
        self.w_lbl.configure(bg=active_bg if self.active_keys['w'] else inactive_bg, fg=active_fg if self.active_keys['w'] else inactive_fg)
        self.a_lbl.configure(bg=active_bg if self.active_keys['a'] else inactive_bg, fg=active_fg if self.active_keys['a'] else inactive_fg)
        self.s_lbl.configure(bg=active_bg if self.active_keys['s'] else inactive_bg, fg=active_fg if self.active_keys['s'] else inactive_fg)
        self.d_lbl.configure(bg=active_bg if self.active_keys['d'] else inactive_bg, fg=active_fg if self.active_keys['d'] else inactive_fg)
    ...
    def send_command(self, raw_cmd=None):
        raw = raw_cmd if raw_cmd else self.entry.get().strip()
        if not raw: return
        parts = raw.split()
